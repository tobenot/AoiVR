...
**绝对禁止目测估算 API 返回的大列表**（说"大约""大概"就是错）。拿到完整响应后（超限自动保存在 `out\` 目录），用 **`convert` 的 `json_query_file`**（沙箱内唯一可靠的程序化统计方式——PowerShell/python/node 在沙箱用户下都无法运行）精确计算：

```
convert(action="json_query_file", data="out\fetch_xxx.txt", filter="location!=offline", count=true)
```

- `json_query_file` 从文件读 JSON（`data` = 相对 workspace 的文件路径），`filter` 单条件（`key==value` / `key!=value`，如 `location!=offline`），`count=true` 直接返回数量。
- 统计好友/世界/通知数量、按字段过滤——一律走这种方式，**数字必须来自计算结果，不允许来自阅读印象**。
- **不要用 `findstr | find /c` 统计 JSON**：API 返回的是单行压缩 JSON，find 数的是"行数"（永远是 1），不是出现次数。
- `json_query`（内联数据）适合小文档提取字段；大文件（几十 KB+）用 `json_query_file`（直接读文件，不用把内容塞进工具参数）。
...
统计命令模板（精确，勿改规则；PowerShell 在沙箱内不可用，一律用 `convert`）：

```
convert(action="json_query_file", data="out\fetch_xxx.txt", filter="location!=offline", count=true)      → 总在线数
convert(action="json_query_file", data="out\fetch_xxx.txt", path="", filter="location!=offline")          → 在线列表（数据）
```

- 在线数 = `location!=offline` 过滤后计数；想拆分公开/私密：`filter="location!=private"` 与 `filter="location==private"` 分别 count。
...
- **报数字前必须程序化统计**（见顶部"列表统计必须程序化"铁律）：响应超限会自动存到 `out\fetch_xxx.txt`，用 `convert` 的 `json_query_file` 读文件精确计数/过滤，**不要**目测、不要说"大约"。
